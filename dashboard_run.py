...
class RunMixin:
    ...
    def _pick_shutdown_emulators(self):
        ...
        wg.restore_geometry(win, "run_pick_emulators_shutdown", default="380x460")
        wg.autosave(win, "run_pick_emulators_shutdown")
        _bind_esc_close(win)
    ...
    def _exec_entry(self, engine, emulator, entry, preset_vars, tracked, context_label=None):
        ...
        # như log thường) - hiển thị NGAY đang chạy tác vụ nào, kèm bối
        # cảnh lịch hẹn giờ/xoay vòng + tài khoản thứ mấy nếu có.
        status_text = f"{context_label} → {name}" if context_label else f"{name} (trên {emulator.name})"
        ...
        try:
            ...
            if self.stop_flag:
                ...
                self._log("warn", f"{prefix}Tác vụ '{name}' bị dừng giữa chừng.", emulator_name=emulator.name)
                return False
    ...
    def _pick_post_login_account(self):
        ...
        win.configure(bg=COL_PANEL)
        wg.restore_geometry(win, "run_pick_account_per_emulator", default="440x480")
        wg.autosave(win, "run_pick_account_per_emulator")
        _bind_esc_close(win)
    ...
    def _pick_account_for_emulator(self, emulator_index, on_done=None):
        ...
        win.configure(bg=COL_PANEL)
        wg.restore_geometry(win, "run_pick_account_post", default="380x460")
        wg.autosave(win, "run_pick_account_post")
        _bind_esc_close(win)
