...
from gui_dialogs import ZoomStepDialog, IfOcrDialog
...
class CanvasMixin:
    ...
    def on_canvas_press(self, event):
        ...
        if self.rect_id:
            ...
            self.rect_id = None
        if (self.pending_action in ("wait_image", "if_image", "wait_vanish", "ocr_text", "if_ocr", "image_region", "popup_pos")) or (self.pending_action is None and "crop" in self.preview_action_mode.get()):
    ...
    def on_canvas_release(self, event):
        ...
        if self.pending_action == "tap":
            if self._tap_edit_idx is not None and 0 <= self._tap_edit_idx < len(self.steps):
                ...
                self._insert_step({"action": "tap", "pos": [rx1, ry1], "repeat": 1, "delay": float(self.spin_default_delay.get() or 1.0), "comment": f"Tap [{int(rx1*100)}%, {int(ry1*100)}%]"})
            ...
            if self._swipe_edit_idx is not None and 0 <= self._swipe_edit_idx < len(self.steps):
                ...
                self._insert_step({"action": "swipe", "from": [rx1, ry1], "to": [rx2, ry2], "duration": 250, "repeat": 1, "delay": float(self.spin_default_delay.get() or 1.0), "comment": f"Swipe"})
            ...
            return
        elif self.pending_action == "swipe_path":
            # KHÁC mọi loại capture khác ở chỗ CẦN NHIỀU cú click liên tiếp
            # (không phải 1 click/kéo là xong): mỗi lần thả chuột chỉ THÊM
            # 1 điểm vào danh sách đang chọn dở rồi vẽ lại marker, KHÔNG
            # gọi _cancel_pending_action() để vẫn ở chế độ chờ click tiếp -
            # xem _finish_swipe_path() (phím Enter) để hoàn tất.
            self._swipe_path_points.append((rx1, ry1))
            self._draw_swipe_path_markers()
            self.drag_start = None
            return
        elif self.pending_action in ("wait_image", "if_image", "wait_vanish"):
            ...
            self._cancel_pending_action()
            return
        elif self.pending_action == "if_ocr":
            # Giống hệt nhánh "ocr_text" ngay dưới (vùng lưu theo TỈ LỆ
            # 0..1, đọc lại MÀN HÌNH HIỆN TẠI mỗi lần chạy) - khác ở chỗ mở
            # IfOcrDialog để hỏi text cần khớp/kiểu khớp/click thay vì chỉ
            # hỏi tên biến, vì if_ocr dùng LÀM ĐIỀU KIỆN IF chứ không chỉ
            # đọc chữ đơn thuần.
            nx1, ny1 = sorted((rx1, rx2))[0], sorted((ry1, ry2))[0]
            nx2, ny2 = sorted((rx1, rx2))[1], sorted((ry1, ry2))[1]
            ...
            if edit_idx is not None:
                ...
                dlg = IfOcrDialog(self.root)
                self.root.wait_window(dlg)
                if dlg.result:
                    r = dlg.result
                    step = {
                        "action": "if_ocr", "box": [nx1, ny1, nx2, ny2],
                        "text": r["text"], "match": r["match"], "click": r["click"],
                        "lang": "vie+eng", "timeout": 3, "scan_interval": float(self.spin_all_scan.get() or 0.10),
                        "repeat": 1, "delay": float(self.spin_default_delay.get() or 1.0),
                        "comment": f"IF OCR \"{r['text']}\"" if r["text"] else "IF OCR (chỉ đọc)",
                    }
                    if r.get("var"):
                        step["var"] = r["var"]
        ...
        if dragged and mode == "tap_only":
            ...
            if mode == "crop_and_tap":
                ...
                self._insert_step({"action": "tap", "pos": [rx1, ry1], "repeat": 1, "delay": float(self.spin_default_delay.get() or 1.0), "comment": f"Tap"})
                threading.Thread(target=self._worker_send_tap, args=(rx1, ry1), daemon=True).start()
    ...
    def _do_crop(self, screen_img, cx, cy, norm_x, norm_y, trigger_tap=False, kind="wait_image", edit_idx=None):
        ...
        if edit_idx is not None and 0 <= edit_idx < len(self.steps):
            ...
            step = capture_tools.build_capture_step(
                ...
                float(self.spin_default_delay.get() or 1.0),
            )
            self._insert_step(step)
