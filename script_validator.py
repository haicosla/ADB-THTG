...
def _check_condition_tree_templates(node, templates_dir, missing_templates):
    """Quét ĐỆ QUY cây điều kiện của if_group (xem
    LogicEngine._eval_condition_node) để tìm ảnh mẫu bị THIẾU nằm trong
    các nút lá 'image' - KHÔNG dùng _check_step_templates() thường được vì
    ảnh trong if_group nằm sâu trong field "tree" lồng nhau, không nằm
    trực tiếp ở "template"/"templates" cấp bước như các action khác."""
    if not isinstance(node, dict):
        return
    node_type = node.get("type")
    if node_type in ("and", "or", "not"):
        for child in (node.get("children") or []):
            _check_condition_tree_templates(child, templates_dir, missing_templates)
...
def _check_step_templates(step, templates_dir, missing_templates):
    ...
    for tpl in (step.get("templates") or []):
        if not os.path.exists(os.path.join(templates_dir, tpl)):
            missing_templates.add(tpl)
    if step.get("action") == "if_group" and step.get("tree"):
        _check_condition_tree_templates(step["tree"], templates_dir, missing_templates)
