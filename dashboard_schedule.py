...
class ScheduleMixin:
    ...
    def _trigger_schedule(self, entry):
        ...
        # tại (Hoạt Động đã bị xoá khỏi Danh Mục) sẽ tự bị bỏ qua.
        activities = [tasks_by_id[tid] for tid in hoat_dong_ids if tid in tasks_by_id]
        if not activities:
    ...
    def _worker_run_schedule(self, schedule_name, emulator_index, accounts, activities):
        ...
        try:
            ...
            if not accounts:
                ...
                _run_activities_with_autologin(None)
    ...
    def _open_schedule_manager(self):
        ...
        win.configure(bg=COL_PANEL)
        wg.restore_geometry(win, "schedule_manager", default="1150x620")
        win.minsize(760, 480)
