...
class RunMixin:
    ...
    def run_selected_steps(self):
        ...
        # Nhớ lại đúng các dòng đang bôi đen NGAY LÚC BẤM nút, để lát sau
        # chạy xong còn bôi đen lại y hệt (xem _on_finish_run) - trước đây
        # KHÔNG lưu lại nên hễ chạy xong là mất chọn, phải tự bôi đen lại
        # từ đầu mới bấm "Chạy Đã Chọn" được lần nữa.
        self._restore_selection_after_run = tuple(str(i) for i in indices)
        ...
        self.engine.step_notifier = lambda sub_idx: self.root.after(
            0, lambda: self._select_step_row_for_run(indices[sub_idx]) if sub_idx < len(indices) else None
        )
    ...
    def _on_finish_run(self, error_msg=None):
        ...
        # Bôi đen lại ĐÚNG các dòng đã chọn trước khi bấm "Chạy Đã Chọn" (nếu
        # có) - làm việc này SAU KHI đã chạy xong hẳn để không bị chính
        # step_notifier ở trên ghi đè mất trong lúc còn đang chạy dở.
        if self._restore_selection_after_run is not None:
            valid_ids = tuple(
                iid for iid in self._restore_selection_after_run
                if iid.isdigit() and int(iid) < len(self.steps)
            )
            if valid_ids:
