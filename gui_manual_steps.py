...
from gui_dialogs import SetVarDialog, IncVarDialog, IfVarDialog, ZoomStepDialog
...
class ManualStepsMixin:
    ...
    def add_if_multi_image(self):
        ...
        self._insert_step({"action": "if_image", "templates": filenames, "timeout": 3, "conf": 0.80, "scan_interval": scan, "repeat": 1, "delay": 0.2, "comment": f"Nếu thấy bất kỳ ({len(filenames)} ảnh)"})
    ...
    def add_multi_image_step(self):
        ...
        filenames = [os.path.basename(p) for p in paths]
        scan = float(self.spin_all_scan.get() or 0.10)
        self._insert_step({"action": "multi_image", "templates": filenames, "timeout": 8, "conf": 0.80, "scan_interval": scan, "click": True, "repeat": 1, "delay": 1.0, "comment": f"Quét song song ({len(filenames)} ảnh)"})
