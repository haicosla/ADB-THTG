...
class ImageTestMixin:
    ...
    def open_image_test_dialog(self):
        ...
        lbl_status.grid(row=5, column=0, columnspan=2, sticky="w", padx=10, pady=(2, 8))

        f_btn = ttk.Frame(win)
        f_btn.grid(row=6, column=0, columnspan=2, pady=(0, 10))
        ...
        while not self._img_test_stop_flag:
            ...
            try:
                if compare:
                    ...
                    pos, score, all_scores = self.adb.find_image_all_modes(tpl_cv, threshold=conf, mode=mode)
                else:
                    pos, score = self.adb.find_image_on_screen(tpl_cv, threshold=conf, mode=mode)
            ...
            if pos:
                self._log_run(
                    "success",
                    f"🧪 [{count}] Tìm thấy ảnh '{name}' tại ({pos[0]:.3f}, {pos[1]:.3f}) - khớp {score:.3f} [{match_mode_short(mode)}]"
                )
            else:
                self._log_run(
                    "warn",
