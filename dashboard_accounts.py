...
class AccountsMixin:
    ...
    def _worker_run_emulator_with_accounts(self, emulator, selected, account_ids):
        ...
        # Vừa TỰ KHỞI ĐỘNG -> chờ thêm cho game lên hẳn, rồi luồng bên dưới chạy
        # 'auto_login' (trước Đăng Xuất) như thường lệ.
        if _auto_started:
    ...
    def _open_account_manager(self):
        ...
        row_widgets = []
        # Thứ tự HIỂN THỊ đầy đủ (tiêu đề Nhóm xen kẽ các dòng) - dùng để
        # vẽ lại ĐÚNG THỨ TỰ mỗi khi bấm nút lọc theo Nhóm, vì pack() sau
        # pack_forget() sẽ đẩy widget xuống CUỐI danh sách đang hiển thị
        # (làm xáo trộn thứ tự Nhóm) nếu không chủ động vẽ lại toàn bộ
        # theo đúng thứ tự này mỗi lần lọc.
        layout_order = []  # list các tuple ("header", widget, group) / ("row", widget, group)

        # Nhóm/nút lọc đang được CHỌN hiện tại (None = "Tất cả") - lưu lại để
        # sau khi ĐỔI THỨ TỰ 1 dòng (⬆️/⬇️) có thể vẽ lại đúng bộ lọc đang
        # xem, thay vì tự động nhảy về "Tất cả".
        current_filter = {"group": None}

        def _apply_filter(g):
            """Vẽ lại TOÀN BỘ theo ĐÚNG layout_order (thay vì chỉ
            pack/pack_forget từng widget riêng lẻ) để giữ đúng thứ tự
            Nhóm dù đã lọc qua lại/đổi thứ tự nhiều lần trước đó. Hoisted
            ra ngoài (không còn nằm trong _rebuild_filter_buttons) để
            _move_row() cũng gọi lại được sau khi đổi chỗ 1 dòng."""
            current_filter["group"] = g
            for kind, w, _gname in layout_order:
                w.pack_forget()
            for kind, w, gname in layout_order:
                # Dùng nhom_var HIỆN TẠI của từng dòng (không phải
                # "group" lúc tạo dòng) để lọc đúng ngay cả khi người
                # dùng vừa gõ đổi Nhóm trong ô combobox trước khi lọc.
                if kind == "row":
                    rw = next((r for r in row_widgets if r["row"] is w), None)
                    row_group = ((rw["nhom_var"].get() or "").strip() or "Chưa phân nhóm") if rw else gname
                    if g is not None and row_group != g:
                        continue
                    w.pack(fill="x", pady=2)
                else:
                    if g is not None and gname != g:
                        continue
                    w.pack(anchor="w", pady=(10, 2), padx=4)

        def _move_row(rid, delta):
            """Đổi chỗ 1 dòng Tài khoản (rid) lên/xuống (delta=-1/+1) với
            dòng LIỀN KỀ CÙNG NHÓM - đây chính là thứ tự sẽ lưu vào
            accounts.json (account_manager.py: thứ tự trong file = thứ
            tự xoay vòng MẶC ĐỊNH khi chọn nhanh 'Tất cả'), nên đổi thứ
            tự ở đây có tác dụng thật, không chỉ đổi hiển thị. CHỈ cho
            đổi chỗ trong CÙNG 1 Nhóm (dùng "group" lúc tạo dòng, không
            phải nhom_var hiện tại) để không xáo trộn các tiêu đề 📁
            Nhóm đang hiển thị theo từng khối - đổi Nhóm cho 1 tài khoản
            thì gõ lại ô 'Nhóm' rồi bấm 💾, không dùng nút ⬆️/⬇️."""
            idx = next((k for k, rw in enumerate(row_widgets) if rw["id"] == rid), None)
            if idx is None:
                return
            jdx = idx + delta
            if not (0 <= jdx < len(row_widgets)):
                return
            rw_a, rw_b = row_widgets[idx], row_widgets[jdx]
            if rw_a["group"] != rw_b["group"]:
                return
            row_widgets[idx], row_widgets[jdx] = rw_b, rw_a
            li = next(k for k, item in enumerate(layout_order) if item[0] == "row" and item[1] is rw_a["row"])
            lj = next(k for k, item in enumerate(layout_order) if item[0] == "row" and item[1] is rw_b["row"])
            layout_order[li], layout_order[lj] = layout_order[lj], layout_order[li]
            _apply_filter(current_filter["group"])
        ...
        def _add_row(acc):
            ...
            rw = {
                ...
                # Nhóm LÚC TẠO DÒNG - chỉ dùng để biết dòng này thuộc phần
                # tiêu đề Nhóm nào (xem group_header_by_name bên dưới), vì
                # nếu tự tay đổi Nhóm ngay trong ô combobox thì dòng vẫn ở
                # NGUYÊN vị trí cũ cho tới khi đóng/mở lại cửa sổ này.
                "group": (acc.get("nhom") or "").strip() or "Chưa phân nhóm",
            }
            ...
            # ⬆️/⬇️ đổi thứ tự với dòng liền kề CÙNG NHÓM - thứ tự này
            # chính là thứ tự lưu vào accounts.json (= thứ tự xoay vòng
            # mặc định), xem _move_row() ở trên để biết vì sao giới hạn
            # trong cùng 1 Nhóm.
            btn_up = tk.Label(row, text="⬆️", bg=COL_PANEL_ALT, fg=COL_TEXT, font=("Segoe UI", 8), cursor="hand2")
            btn_up.pack(side="left", padx=(4, 0))
            btn_up.bind("<Button-1>", lambda e, rid=rid: _move_row(rid, -1))
            btn_down = tk.Label(row, text="⬇️", bg=COL_PANEL_ALT, fg=COL_TEXT, font=("Segoe UI", 8), cursor="hand2")
            btn_down.pack(side="left", padx=(1, 4))
            btn_down.bind("<Button-1>", lambda e, rid=rid: _move_row(rid, 1))
            ...
            layout_order.append(("row", row, rw["group"]))

        # ----- TÁCH RIÊNG các dòng THEO TỪNG NHÓM (giống '📁 Mục' ở popup
        # 'Chọn Hành Động Để Chạy') - mỗi Nhóm có 1 tiêu đề riêng, dễ rà
        # soát hơn hẳn 1 danh sách phẳng khi có nhiều Tài khoản thuộc nhiều
        # Nhóm khác nhau. Tài khoản chưa gán Nhóm gom vào 'Chưa phân nhóm'.
        # Sắp theo thứ tự Nhóm xuất hiện ĐẦU TIÊN trong self.accounts (giữ
        # thứ tự quen thuộc thay vì xáo trộn theo A-Z).
        group_header_by_name = {}
        _seen_groups = []
        for acc in self.accounts:
            g = (acc.get("nhom") or "").strip() or "Chưa phân nhóm"
            if g not in _seen_groups:
                _seen_groups.append(g)
        accounts_by_group = {g: [] for g in _seen_groups}
        for acc in self.accounts:
            g = (acc.get("nhom") or "").strip() or "Chưa phân nhóm"
            accounts_by_group[g].append(acc)

        for g in _seen_groups:
            header = tk.Label(inner, text=f"📁 {g}", bg=COL_PANEL, fg=COL_TEXT_MUTED,
                               font=("Segoe UI", 9, "bold"))
            header.pack(anchor="w", pady=(10, 2), padx=4)
            group_header_by_name[g] = header
            layout_order.append(("header", header, g))
            for acc in accounts_by_group[g]:
                _add_row(acc)
        ...
        def _rebuild_filter_buttons():
            ...
            # _apply_filter() giờ được định nghĩa Ở NGOÀI (phía trên, cạnh
            # row_widgets/layout_order) để _move_row() (nút ⬆️/⬇️ đổi thứ
            # tự từng dòng) cũng gọi lại được sau khi đổi chỗ 1 dòng, không
            # còn là hàm lồng riêng trong _rebuild_filter_buttons() nữa.
    ...
    def _open_single_login_dialog(self):
        ...
        # QUAN TRỌNG: mỗi FlowBar con (1 cho mỗi Nhóm, tạo trong _populate
        # bên dưới) tự xếp nút bằng place() nên KHÔNG tự nới rộng theo nút
        # con - nếu để canvas tự đo kích thước mặc định (rất nhỏ, ~1px) thì
        # các nút coi như bị "giam" trong vùng 1px đó và KHÔNG hiển thị ra
        # ngoài (dù vẫn được tạo ra bình thường). Phải CHỦ ĐỘNG ép chiều
        # rộng của `inner` bằng đúng chiều rộng hiển thị của canvas mỗi khi
        # canvas đổi kích thước - mỗi FlowBar con (pack fill='x' bên trong
        # inner) sẽ tự nhận đúng chiều rộng qua cơ chế pack bình thường.
        ...
        def _account_label(a):
            return a.get("ten_hien_thi") or a.get("username") or a.get("id")
        ...
        def _populate(group=None, text_filter=""):
            # TÁCH RIÊNG THEO TỪNG NHÓM (giống '👥 Quản Lý Tài Khoản' và
            # '📁 Mục' ở popup 'Chọn Hành Động Để Chạy') - mỗi Nhóm 1 tiêu
            # đề + 1 hàng nút riêng, thay vì 1 hàng nút phẳng gộp chung tất
            # cả Tài khoản kèm '[Nhóm]' ghi trong nhãn như trước đây. Vẽ
            # lại TOÀN BỘ từ đầu mỗi lần lọc/gõ chữ (đơn giản, không cần
            # theo dõi thứ tự cũ vì popup này chỉ dùng để CHỌN, không sửa
            # trực tiếp Nhóm của Tài khoản tại đây).
            ...
            seen_groups = []
            for a in filtered:
                g = (a.get("nhom") or "").strip() or "Chưa phân nhóm"
                if g not in seen_groups:
                    seen_groups.append(g)
            by_group = {g: [] for g in seen_groups}
            for a in filtered:
                g = (a.get("nhom") or "").strip() or "Chưa phân nhóm"
                by_group[g].append(a)

            if not filtered:
                tk.Label(inner, text="(Không có Tài khoản nào khớp)", bg=COL_PANEL,
                         fg=COL_TEXT_MUTED).pack(anchor="w", pady=10, padx=4)
                return

            for g in seen_groups:
                tk.Label(inner, text=f"📁 {g}", bg=COL_PANEL, fg=COL_TEXT_MUTED,
                         font=("Segoe UI", 9, "bold")).pack(anchor="w", pady=(10, 2), padx=4)
                flow = FlowBar(inner, bg=COL_PANEL)
                flow.pack(fill="x")
                for a in by_group[g]:
                    aid = a.get("id")
                    is_sel = (aid == selected["id"])
                    btn = RoundedButton(flow, _account_label(a), command=lambda aid=aid: _select_account(aid),
    ...
    def _worker_quick_login(self, emulator, account):
        ...
        # "Tự Login" (Hoạt Động id 'auto_login') chạy TRƯỚC Đăng Xuất - giống
        # hệt luồng Xoay Vòng/Hẹn Giờ (xem _worker_run_emulator_with_accounts ở trên):
        # phải chắc đang ở TRONG GAME thì 'account_logout' mới bấm được nút
        # Đăng Xuất. Đặc biệt quan trọng khi giả lập vừa được TỰ BẬT (đang
        # ở màn hình chủ Android, game chưa mở) - nếu bỏ qua bước này thì
        # mọi IF trong 'account_logout' đều SAI và không có gì xảy ra.
        # Thất bại KHÔNG chặn (chỉ cảnh báo) - vẫn Đăng Xuất/Đăng Nhập như
        # bình thường, đúng quy ước của luồng xoay vòng.
        if getattr(self, "auto_login_var", None) and self.auto_login_var.get():
            auto_login_entry = task_registry.find_task(self.tasks, "auto_login")
            if not auto_login_entry:
                self._log("warn", "Đã bật 'Tự Login' nhưng chưa có Hoạt Động với id 'auto_login' (xem "
                                   "'📖 Hướng Dẫn') - Log Nhanh sẽ Đăng Xuất/Đăng Nhập luôn.",
                           emulator_name=emulator.name)
            else:
                self._log("info", "⚡ Log Nhanh: chạy 'Tự Login' để vào game trước khi đổi tài khoản...",
                           emulator_name=emulator.name)
                ok_pre = self._exec_entry(engine, emulator, auto_login_entry, preset_vars=None, tracked=False)
                if self._is_stop_requested(emulator.index):
                    self.root.after(0, lambda: self._finish_quick_login(emulator.index))
                    return
                if not ok_pre:
            ...
            self._log("info", "⚡ Log Nhanh: đang TẮT 'Tự Login' ở thanh trên - bỏ qua bước vào game trước khi "
                               "Đăng Xuất (tick 'Tự Login' nếu muốn tự vào game trước).",
                       emulator_name=emulator.name)
        ...
        if ok:
            # mark_run_by_id - an toàn khi nhiều giả lập/luồng Log Nhanh
            # khác đang ghi accounts.json cùng lúc (xem docstring trong
            # account_manager.py).
            account_manager.mark_run_by_id(account.get("id"))
