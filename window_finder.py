...
class WindowFinder:
    ...
    def embed_as_overlay_child(self, hwnd, screen_x, screen_y, width, height):
        """Biến 1 cửa sổ NGOÀI (vd popup Tkinter overrideredirect) thành
        CON (child) THẬT SỰ của cửa sổ LDPlayer chính (main_hwnd), đặt tại
        đúng vị trí (screen_x, screen_y) trên MÀN HÌNH (tự quy đổi sang
        toạ độ TƯƠNG ĐỐI so với main_hwnd - toạ độ cửa sổ con luôn tính
        theo gốc client area của cửa sổ cha, không phải theo màn hình).

        Nhờ vậy overlay LUÔN nằm THẬT SỰ BÊN TRONG cửa sổ LDPlayer: tự di
        chuyển/thu nhỏ/khôi phục/ẩn CÙNG với cửa sổ đó (đúng cơ chế
        Windows quản lý cửa sổ cha-con), không bao giờ "lạc" ra vị trí cố
        định trên Desktop hay đè nhầm lên app khác đang ở phía trước -
        khác hẳn cách cũ (overlay "topmost" tách rời, chỉ ước lượng toạ độ
        MỘT LẦN lúc hiện ra rồi thả nổi độc lập, không còn liên hệ gì với
        cửa sổ LDPlayer sau đó).

        Trả về True nếu nhúng thành công, False nếu KHÔNG (main_hwnd chưa
        có/không hợp lệ, hoặc gọi API win32 bị lỗi vì lý do nào đó) - nơi
        gọi (xem gui.py/dashboard_misc.py::_show_ingame_popup) nên tự
        fallback về overlay nổi thông thường khi hàm này trả về False,
        để KHÔNG BAO GIỜ vì lỗi win32 mà mất luôn thông báo."""
        if not self.main_hwnd or not win32gui.IsWindow(self.main_hwnd):
            return False
        try:
            main_origin = win32gui.ClientToScreen(self.main_hwnd, (0, 0))
            rel_x = int(screen_x - main_origin[0])
            rel_y = max(0, int(screen_y - main_origin[1]))

            # Đổi style từ cửa sổ nổi (WS_POPUP, không viền vì overrideredirect
            # đã tự bỏ WS_CAPTION) sang cửa sổ CON (WS_CHILD) - bắt buộc phải
            # đổi style TRƯỚC khi SetParent, đúng thứ tự API win32 tiêu chuẩn
            # khi "nhúng" 1 cửa sổ ngoài vào làm con của cửa sổ khác.
            style = win32gui.GetWindowLong(hwnd, win32con.GWL_STYLE)
            style = (style & ~win32con.WS_POPUP & ~win32con.WS_CAPTION) | win32con.WS_CHILD
            win32gui.SetWindowLong(hwnd, win32con.GWL_STYLE, style)
            win32gui.SetParent(hwnd, self.main_hwnd)
            # HWND_TOP: đẩy lên ĐẦU thứ tự hiển thị trong SỐ CÁC CON của
            # main_hwnd (cùng cấp với render_hwnd - khung game) để overlay
            # luôn hiện ĐÈ LÊN TRÊN khung game, không bị khung game vẽ đè.
            win32gui.SetWindowPos(hwnd, win32con.HWND_TOP, rel_x, rel_y, int(width), int(height),
